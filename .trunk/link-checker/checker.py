...
ALLOW_LIST_URLS = ["https://marketplace.visualstudio.com/items?itemName=trunk.io", "https://static.trunk.io/assets/vscode\_init\_trunk.png", "https://static.trunk.io/assets/vscode\_side\_panel.png", "https://static.trunk.io/assets/vscode\_hide\_explorer\_panel.jpg", "https://static.trunk.io/assets/vscode\_doc\_links.png", "https://static.trunk.io/assets/vscode\_ignore\_issue.gif"]
...
def check_fragment(source_path, link_dest, fragment):
    if fragment in ALLOW_LIST_FRAGMENTS:
        return True
    fragment_regex = r"# {}".format(fragment.replace("-", "."))
    with open(link_dest, "r") as target:
        if re.search(fragment_regex, target.read(), re.IGNORECASE) is None:
            print(">>> Could not detect fragment '{}' in file {} pointing to from {}".format(fragment, link_dest, source_path))
            return False
        return True
...
def validate_link(path, url):
    ...
    if url.startswith("http"):
        stripped_url = urlparse(url, allow_fragments=False)._replace(query="", fragment="").geturl()
    ...
    if fragment_index > 0:
        ...
        # print(url, fragment_index, from_file_path, os.path.exists(from_file_path), from_root_path, os.path.exists(from_root_path))
...
for path in Path('check').rglob('*.md'):
    # print(path.name)
    with open(path, "r") as file:
        ...
        for match in matches:
          if not validate_link(path, match[1]):
              fail = True
              print("{}: Could not find url: '{}' with description: '{}'".format(path, match[1], match[0]))
