...
Simply configure `.trunk/user.yaml` as you would for `.trunk/trunk.yaml`. Now you can add additional linters, enable [actions](../actions/overview.md), or specify [default command options](doc:reference-trunk-yaml#cli), without impacting the way other developers run `trunk`.
