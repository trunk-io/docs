...
* [Optimistic Merging](/broken/pages/k2ELH7R7uPCqaNixtx1z) uses the successful test of a pull request later in the queue to validate all the PRs ahead of it in the queue, allowing the entire sequence to be merged without waiting for the earlier PRs to finish testing.
* [Batching](/broken/pages/mD7GJVG04RV7TvzVP6TY) allows the queue to _test_ multiple PRs in a single CI job to save time and resources. After the test passes, the PRs in the batch are still merged one by one.
...
**Recommended approach:** Use [PR Prioritization](/broken/pages/KBGLXshIgaxHjvuU5rpy) to fast-track your PR through the queue while still validating it:
