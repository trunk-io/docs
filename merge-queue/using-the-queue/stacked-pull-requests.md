...
6. **Sequential merge** — Trunk merges each member PR into the temporary branch starting from the PR closest to the merge queue branch. If any merge conflicts, the operation fails and you'll need to resolve conflicts before retrying.
...
Trunk reacts to your `/trunk stack` comment with a 👍 on success or a 👎 on failure, along with an explanation of what went wrong.
...
1. Each member PR is automatically closed by Trunk — their code is already in the target branch via the stacked PR.
