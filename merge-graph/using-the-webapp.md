...
The PR Details panel has a dropdown menu (labeled "**..."**) with actions. To cancel a PR you can use this menu to remove the PR from the queue:
...
A PR details page will display a complete history of a PR - state transitions, associated test runs, etc. We also offer the uploaded impacted targets for a particular PR if they were provided regardless of queue mode (`Single` or `Parallel`), and a resubmit button.
