...
You can define linters right in your `.trunk/trunk.yaml` file in your repo. These definitions have the same configurable parameters as in our [public plugins repo](https://github.com/trunk-io/plugins/blob/main/CONTRIBUTING.md) or [your own plugins repo](../cli/getting-started/configuration/plugins/external-repositories.md).
...
You can also define simple linters inline using tools like `grep`. This linter will grep against your custom regex pattern, format the output using sed, and then parse the output into pattern groups using a [regex output](../cli/getting-started/configuration/lint/output.md#regex) for Trunk Code Quality to report.
...
To see the configurable fields available [Linter Definition Reference](../cli/getting-started/configuration/lint/definitions.md).
