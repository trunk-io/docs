...
By default, Trunk will try to automatically initialize itself in single-player mode. This means that it'll create a Trunk configuration that is hidden from git, which allows you to try it out [without Trunk's versioning powers](broken-reference).
...
Check out how to [install the CLI](docs:install), [set it up in CI](docs:ci), [ignore issues](docs:ignoring-issues), and set up [Custom Linters](doc:check-custom-linters).
